...
namespace Rally { namespace View {
    ...
    MotionBlurView::~MotionBlurView() {
        motionBlurBlendShaderParameters.setNull();
        //blurHorizontalShaderParameters.setNull();
        //blurVerticalShaderParameters.setNull();
    }

    void MotionBlurView::attachTo(Ogre::Viewport* viewport, const Rally::Model::Car* car) {
        this->viewport = viewport;
        this->car = car;

        compositorInstance = Ogre::CompositorManager::getSingleton().addCompositor(viewport, "compositors/MotionBlurCompositor");
        if(compositorInstance == NULL) {
            throw std::runtime_error("Compositor for motion blur could not be instantiated.");
        }

        compositorInstance->addListener(this);
    }

    void MotionBlurView::setEnabled(bool enabled) {
        Ogre::CompositorManager::getSingleton().setCompositorEnabled(viewport, "compositors/MotionBlurCompositor", enabled);
    }

    void MotionBlurView::detach() {
        motionBlurBlendShaderParameters.setNull();
        //blurHorizontalShaderParameters.setNull();
        //blurVerticalShaderParameters.setNull();
        Ogre::CompositorManager::getSingleton().removeCompositor(viewport, "compositors/MotionBlurCompositor");
    }

    void MotionBlurView::notifyMaterialSetup(Ogre::uint32 compositorPassId, Ogre::MaterialPtr& clonedMaterial) {
        if(compositorPassId == 100) {
            motionBlurBlendShaderParameters = clonedMaterial->getBestTechnique()->getPass(0)->getFragmentProgramParameters();
    ...
    void MotionBlurView::notifyMaterialRender(Ogre::uint32 compositorPassId, Ogre::MaterialPtr& clonedMaterial) {
        if(compositorPassId == 100) {
            float effectFactor = car->getEffectFactor();

           }/*
            motionBlurBlendShaderParameters->setNamedConstant("effectFactor", effectFactor);
    ...
    /*void MotionBlurView::recalculateBlur(float effectFactor) {

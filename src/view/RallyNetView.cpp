...
namespace Rally { namespace View {
    ...
    namespace {
        ...
        void writeVector3toPacket(char* packet, const Rally::Vector3 & vector) {
            memcpy(packet + 0*4, static_cast<void*>(static_cast<float>(vector.x)), 4);
            memcpy(packet + 1*4, static_cast<void*>(static_cast<float>(vector.y)), 4);
            memcpy(packet + 2*4, static_cast<void*>(static_cast<float>(vector.z)), 4);
        }
    }

    RallyNetView::RallyNetView(const RallyNetView_NetworkCarListener & listener)
        : socket(0),
        lastSentPacketId(0),
        listener(listener) {
    }
    ...
    void RallyNetView::initialize(const std::string & serverAddress, unsigned short serverPort) {
    #ifdef PLATFORM_WINDOWS
        WSADATA WsaData;
...
#endif
        {
            throw std::exception("Could not make socket non-blocking!");
        }
    ...
    void RallyNewView::updatePutCar() {
        char packet[36];
        ...
        if(::send(socket, packet, sizeof(packet), 0x00000000) < 0) {
    ...
    void RallyNetView::updateGetCars() {
        ...
        while(true) {
            ...
            if(receivedBytes == 40 && packet[0] == 1) { // complete packetType == 1
                ...
                if(sendingClientIterator != map::end) {
                     if(sendingClientIterator->lastPositionSequenceId > sequenceId) {
                        // Possible wrap-around?
                        if(sendingClientIterator->lastPositionSequenceId > USHRT_MAX/2 && sequenceId < USHRT_MAX/2) {
                ...
                RallyNetView_InternalClient& internalClient = (sendingClientIterator == map::end) ? clients[playerId] : sendingClientIterator;
